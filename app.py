  ...
  async function fetchAllAds() {
    ...
    while (page <= totalPage) {
      const r = await fetch('/api/v1/product/listing/shopee/queryAdCampaignShopInfoPage.json', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pageNo: page, pageSize: 100, campaignStatus: 'ongoing' })
      });
      const d = await r.json();
      if (d.code !== 0) break;
      totalPage = d.data?.totalPage || 1;
      (d.data?.rows || []).forEach(ad => {
        if (ad.biddingMethod === 'autoRoas') ads.push(ad);
      });
      page++;
    }
    return ads;
  ...
  async function fetchAdsWithDateRange(days) {
    ...
    while (page <= totalPage) {
      const r = await fetch('/api/v1/product/listing/shopee/queryAdCampaignShopInfoPage.json', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pageNo: page, pageSize: 100, campaignStatus: 'ongoing', startDateStr: startStr, endDateStr: endStr })
      });
      const d = await r.json();
      if (d.code !== 0) break;
      totalPage = d.data?.totalPage || 1;
      (d.data?.rows || []).forEach(ad => {
        if (ad.biddingMethod === 'autoRoas') {
...
def _fetch_ads_range(days=None):
    ...
    body = {"pageNo": 1, "pageSize": 200, "campaignStatus": "ongoing"}
    if days:
        today = datetime.now()
    ...
    while True:
        ...
        for row in rows:
            if row.get("biddingMethod") == "autoRoas":
                ads.append(row)
