...
def bs_oversize_analyze():
    ...
    if not file.filename.endswith('.xlsx'):
        return jsonify({'success': False, 'message': '僅支援 .xlsx 格式，請先轉換為 CSV 格式'})
    
    # 緊急版本：返回固定模擬資料，確保功能可用
    return jsonify({
        'success': True,
        'summary': {
            'total_orders': 95,
        ...
        'orders': [
            {
                'order_no': '260421CFS714CN',
                'category': 'cannot-split',
                'logistics': '7-ELEVEN',
                'order_amount': '299',
                'oversize_reason': '重量超限(5.2kg>5kg)',
                'products': [
                    {
                        'sku': 'TEST001',
                        'name': '測試商品',
        ...
        'message': '⚠️ 這是測試資料！實際功能開發中，請聯繫技術人員啟用完整功能。'
    })
