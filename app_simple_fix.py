        ...
        for i, row in enumerate(rows):
            ...
            if img_buf:
                try:
                    ...
                    if img_url:
                        # ── 修復 Google Drive 連結格式 ──
                        fixed_url = img_url
                        if "drive.google.com/file/d/" in img_url and not img_url.endswith("/view"):
                            import re
                            m = re.search(r'drive\.google\.com/file/d/([a-zA-Z0-9_-]+)', img_url)
                            if m:
                                fixed_url = f"https://drive.google.com/file/d/{m.group(1)}/view"
                        
                        cell = ws.cell(r, 18, "點擊看圖")
                        cell.hyperlink = fixed_url
